...
//create UDP instance
WiFiUDP udp0; // RFID UDP
WiFiUDP udp1;
WiFiUDP udp2;
String RFID_IP = "Not Set";
String Input1_IP = "Not Set";
String Input2_IP = "Not Set";
String Port0 = "Not Set"; // RFID Port
...
unsigned long startTimeRelayOne = 0;
unsigned long startTimeRelayTwo = 0;
unsigned long currentTimeRelayOne = 0;
unsigned long currentTimeRelayTwo = 0;
...
String processor(const String &var)
{
  ...
  else if (var == "PH_RFID_IP")
    return RFID_IP;
  else if (var == "PH_Port0")
    return Port0;
...
void startAP()
{
  ...
  logOutput(WiFi.softAPConfig(local_ap, gateway, subnet_ap) ? "Ready" : "Failed!");
  delay(100);
...
void setup()
{
  ...
  Serial.begin(115200);
  delay(100);
  Serial1.begin(38400); // 13.56 Mhz MOD-RFID1356MIFARE Olimex reader
  ...
  if (networkRead.size() > 8)
  {
    ...
    if (SPIFFS.exists("/outputs.txt"))
    {
      ...
      {
        Timer1 = values_config[0];
      }
      else
      {
        values_config[0] = "Default Timer 1: " + Timer1;
      }
      if (values_config[1] != NULL || values_config[1].length() != 0)
      {
        Timer2 = values_config[1];
      }
      else
      {
        values_config[1] = "Default Timer 2: " + Timer2;
      }
    ...
    {
      String values_config[3];
      File configRead = SPIFFS.open("/rfid.txt", "r");
      if (!configRead)
        logOutput("ERROR_INSIDE_CONFIG_FILE_READ ! Couldn't open /configFile to read values!");

      fileReadLines(configRead, values_config);
      if (values_config[0] == "ON")
      {
        activateRFID = true;
      }
      else
      ...
      if (values_config[1] != NULL || values_config[1].length() != 0)
      {
        RFID_IP = values_config[1];
      }
      else
      {
        values_config[1] = "Not Set";
      }
      if (values_config[2] != NULL || values_config[2].length() != 0)
      {
        Port0 = values_config[2];
      }
      else
      {
        values_config[2] = "Not Set";
      }
      logOutput("Present RFID Configuration: ");
      for (int k = 0; k < 3; k++)
      {
        logOutput((String)(k + 1) + ": " + values_config[k]);
      }
    ...
    server.on("/relay1/on", HTTP_GET, [](AsyncWebServerRequest *request)
              {
                if (userFlag)
                {
                  ...
                  else
                  {
                    ...
                    logOutput(" Relay 1 will automatically close in " + Timer1 + " seconds !");
                  }
    ...
    server.on("/relay2/on", HTTP_GET, [](AsyncWebServerRequest *request)
              {
                if (userFlag)
                {
                  ...
                  else
                  {
                    ...
                    logOutput("Relay 2 will automatically close in " + Timer2 + " seconds !");
                  }
    ...
    server.on("/rfid-settings", HTTP_GET, [](AsyncWebServerRequest *request)
              {
                if (userFlag)
                {
                  if (!request->authenticate(value_login[0].c_str(), value_login[1].c_str()))
                    return request->requestAuthentication(NULL, false);
                  request->send(SPIFFS, "/rfid.html", "text/html", false, processor);
                }
                else
                {
                  request->send(SPIFFS, "/rfid.html", "text/html", false, processor);
    ...
    server.on("/inputs-outputs", HTTP_POST, [](AsyncWebServerRequest *request)
              {
                ...
                {
                  String b = request->arg("getTimer1");
                  String d = request->arg("getTimer2");

                  if (b.length() != 0)
                  {
                    Timer1 = b;
                    logOutput((String) "POST[getTimer1]: " + Timer1);
                  }
                  if (d.length() != 0)
                  {
                    Timer2 = d;
                    logOutput((String) "POST[getTimer2]: " + Timer2);
                  }

                  File configWrite = SPIFFS.open("/outputs.txt", "w");
                  if (!configWrite)
                    logOutput("ERROR_INSIDE_DELAY_POST ! Couldn't open file to save DELAY !");
                  configWrite.println(Timer1);
                  configWrite.println(Timer2);
                  configWrite.close();
    ...
    server.on("/rfid-settings", HTTP_POST, [](AsyncWebServerRequest *request)
              {
                if (request->hasArg("send_data"))
                {
                  String a = request->arg("getInput0");
                  String b = request->arg("getPort0");
                  if (a.length() != 0)
                  {
                    RFID_IP = a;
                    logOutput((String) "POST[getInput0]: " + RFID_IP);
                  }
                  if (b.length() != 0)
                  {
                    Port0 = b;
                    logOutput((String) "POST[getPort0]: " + Port0);
                    command = true;
                  }

                  File configWrite = SPIFFS.open("/rfid.txt", "w");
                  if (!configWrite)
                    logOutput("ERROR_INSIDE_DELAY_POST ! Couldn't open file to save DELAY !");
                  configWrite.println("ON");
                  configWrite.println(RFID_IP);
                  configWrite.println(Port0);
                  configWrite.close();

                  activateRFID = true;

                  request->redirect("/rfid-settings");
                }
                else
                {
                  request->redirect("/rfid-settings");
...
unsigned long timerDelta = 0;
unsigned long startBuzzer = 0;
bool counting1 = false;
bool counting2 = false;
bool udpStarted0 = false;
...
void loop()
{
  ...
  {
    if ((currentTimeRelayOne - startTimeRelayOne) > (Timer1.toInt() * 1000))
    {
      digitalWrite(RELAY1, LOW);
      status1 = "OFF";
  ...
  {
    if ((currentTimeRelayTwo - startTimeRelayTwo) > (Timer2.toInt() * 1000))
    {
      digitalWrite(RELAY2, LOW);
      status2 = "OFF";
  ...
  if (activateRFID)
  {
    ...
    if (rf_id.length() == 10 && rf_id != "0000000000")
    {
      ...
      // send packet to server
      if (RFID_IP.length() != 0 && RFID_IP != "Not Set" && udpStarted0)
      {
        udp0.beginPacket(RFID_IP.c_str(), Port0.toInt());
        udp0.write(buffer, sizeof(buffer));
        delay(30);
        logOutput(udp0.endPacket() ? (String) "RFID: " + foobar + " was sent over UDP." : "WARNING: RFID not sent.");
        memset(buffer, 0, 40);
        digitalWrite(BUZZER, HIGH);
        startBuzzer = millis();
        delay(100);
